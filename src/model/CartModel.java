	...
	public static CartItem getCartItem(String idCustomer, String idProduct) {
		...
		try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(query)) {
			...
			if (rs.next()) {
				...
				return new CartItem(rs.getString("idCustomer"), rs.getString("idProduct"), rs.getInt("count"), product);
			}
		} catch (SQLException e) {
	...
	public static List<CartItem> getCartItemsByCustomer(String idCustomer) {
		...
		try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(query)) {
			...
			while (rs.next()) {
				...
				CartItem item = new CartItem(rs.getString("idCustomer"), productId, rs.getInt("count"), product);
				cartItems.add(item);
			}
		} catch (SQLException e) {
